        ...
        for (int i = 0; i < 11; i++) {
            for (int j = 0; j < 13; j++) {
                ...
                if ((p[i] <= 14) && (p[i] != 10)) {
                    ...
                    f[i][j] = Math.asin(1/Math.pow(Math.E, Math.pow(Math.cos(Math.pow(Math.pow((3*(Math.atan((x[j]+1.5)/19)-1)),(x[j]/(2+x[j]))),((0.25+(Math.pow((Math.tan(x[j])/Math.PI),3))/Math.pow(Math.E, (Math.asin((x[j]+1.5)/19))))))),2)));
                }
            }
        }
        for (int i = 0; i < 11; i++) {
            for (int j = 0; j < 13; j++)    {
                System.out.print(String.format("%8.2f",f[i][j]));
            }
        }
    }
}
